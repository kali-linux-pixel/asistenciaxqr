...
<div class="chat-layout">
    <!-- Cabecera del Chat para el Director -->
    <div class="chat-header">
        <a href="<?php echo URLROOT; ?>/mensajes/index" class="btn-back" title="Volver a la bandeja">
            <i class="fa-solid fa-chevron-left"></i>
        </a>
        <div class="avatar">
            <?php echo mb_strtoupper(mb_substr($data['destinatario']['nombre'], 0, 2)); ?>
        </div>
        <div>
            <div style="font-weight:700; font-size:1rem;"><?php echo htmlspecialchars($data['destinatario']['nombre']); ?></div>
            <div style="font-size:0.72rem; opacity:0.8;">Docente de la Institución</div>
        </div>
    </div>
    ...
    <div class="chat-messages" id="chatMessages">
        <?php if(empty($data['conversacion'])): ?>
            <div style="text-align:center; margin:auto; padding:2rem; color:#64748b;">
                <i class="fa-solid fa-message" style="font-size:3rem; color:#cbd5e1; margin-bottom:15px;"></i>
                <h3 style="font-weight:700; color:#1e293b; margin-bottom:5px;">Inicia la conversación</h3>
                <p style="font-size:0.85rem; max-width:320px; margin:auto;">Envía una instrucción o un saludo al docente <?php echo htmlspecialchars($data['destinatario']['nombre']); ?>.</p>
            </div>
        <?php else: ?>
    ...
    <div class="chat-input-area">
        <form action="<?php echo URLROOT; ?>/mensajes/enviar" method="POST" class="chat-form">
            ...
            <input type="text" name="contenido" placeholder="Escribe un mensaje al docente..." required autocomplete="off" id="msgInput">
